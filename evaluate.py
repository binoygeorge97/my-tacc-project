...
def visualize_system_plots(inputs, targets, preds, dataset_name="microgrid", n_plot=3, max_channels=4, custom_title=""):
    ...
    # Save locally and RETURN the path instead of logging immediately
...
def run_evaluation(model, Ad, Bd, d_model, n_layers, dataset_name="microgrid", custom_title=""):
    ...
    # Return the file path generated by the visualizer
    return visualize_system_plots(inputs_u, targets_y, preds_y, n_plot=3, dataset_name=dataset_name, custom_title=custom_title)
...
if __name__ == "__main__":
    ...
    # --- NEW: List to collect all generated plot paths ---
    generated_plots = []
    ...
    for filepath in model_files:
        ...
        # --- NEW: Catch the returned path and store it ---
        ...
        if saved_path:
            generated_plots.append(saved_path)
        
    # --- NEW: Upload ALL plots simultaneously as a gallery ---
    if generated_plots:
        print(f"\n[*] Uploading {len(generated_plots)} plots to Weights & Biases...")
        wandb.log({"Evaluation_Gallery": [wandb.Image(p) for p in generated_plots]})
